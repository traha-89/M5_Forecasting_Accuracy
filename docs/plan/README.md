...
| `reports/baselines.csv` | P4 | P6, P7 | `baseline`, `fold`, `scope` (`full` / `CA_1`), `wrmsse`, `rmsse`, `wmae`, `bias_pct` |
| `reports/dead_series.csv` | P4 | P7, P8 | `id` of every series with zero sales in `d_1854`–`d_1913` |
| `src/features.py` | P5 | P6, P7, P8 | `build_features(store_id, …)` and `build_encodings(train_slice)` — the single definition used by pilot and full run alike |
| `data/processed/features/store_id=<STORE>/part.parquet` | P5 | P6, P7, P8 | One partition per store. Engineered features `float32`, `sales` `int16`, ids `category`. Excludes target encodings, which are fold-dependent and computed at fit time via `build_encodings` |
...
Fold definitions, used identically by P4, P6, and P7. The scale (naive-baseline) and weight
windows are **fold-local** — `src/metrics.py`'s `training_end`, `weight_start` and `weight_end`
must be passed per fold, never left at their module defaults, or a fold is scored using days after
...
| Holdout (P7) | `d_1` – `d_1913` | `d_1914` – `d_1941` | 1913 | `d_1886` – `d_1913` |

Models that early-stop use an inner split: for a fold predicting `[a, b]`, stop on `[a-28, a-1]`
and fit on `d_1` – `d_{a-29}`. Never early-stop on the days being scored.
